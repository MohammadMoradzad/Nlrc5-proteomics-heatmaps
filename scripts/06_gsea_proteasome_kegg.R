...
output_dir <- "outputs/gsea/proteasome"
...
kegg_links <- KEGGREST::keggLink("mmu", "path:mmu03050")
...
proteasome_genes <- unique(map_tbl$SYMBOL)

cat("Proteasome genes in KEGG:", length(proteasome_genes), "\n")
...
make_proteasome_gsea_plot <- function(
  ranks,
  pathway_genes,
  highlight_genes,
  comparison_name
  ...
  if (length(pathway_genes) < 10) {
    stop(paste("Too few KEGG proteasome genes overlap for", comparison_name))
  }

  es_df <- compute_running_es(ranks, pathway_genes, gsea_weight = 1)

  fg <- fgseaMultilevel(
    pathways = list(Proteasome_KEGG = pathway_genes),
    stats = ranks,
    minSize = 10,
    maxSize = 500
  )
  ...
  cat("Highlighted genes present in KEGG proteasome pathway:\n")
  ...
  tick_line <- "Black ticks = KEGG proteasome genes   |   Red ticks = highlighted proteasome genes"
  ...
  p <- ggplot(es_df, aes(x = Rank, y = RunningES)) +
    ...
    labs(
      title = paste0("KEGG GSEA: Proteasome (", comparison_name, ")"),
...
plot_N5FL <- make_proteasome_gsea_plot(
  ranks = res_N5FL_vs_V$ranks,
  pathway_genes = proteasome_genes,
  highlight_genes = highlight_genes,
  comparison_name = "N5FL_vs_V"
)

plot_N5CA <- make_proteasome_gsea_plot(
...
ggsave(
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5FL_vs_V.pdf"),
  plot_N5FL$plot,
  width = 11,
  height = 7.2
)
...
ggsave(
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5FL_vs_V.png"),
  plot_N5FL$plot,
  width = 11,
  height = 7.2,
  dpi = 600
...
ggsave(
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5CA_vs_V.pdf"),
  plot_N5CA$plot,
  width = 11,
  height = 7.2
)
...
ggsave(
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5CA_vs_V.png"),
  plot_N5CA$plot,
  width = 11,
  height = 7.2,
  dpi = 600
...
write_tsv(
  plot_N5FL$fgsea,
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5FL_vs_V_fgsea_results.tsv")
)

write_tsv(
  plot_N5CA$fgsea,
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5CA_vs_V_fgsea_results.tsv")
)

write_tsv(
  plot_N5FL$highlighted,
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5FL_vs_V_highlighted_genes.tsv")
)

write_tsv(
  plot_N5CA$highlighted,
  file.path(output_dir, "Proteasome_KEGG_GSEA_N5CA_vs_V_highlighted_genes.tsv")
)
